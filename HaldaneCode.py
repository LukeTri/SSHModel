...
#large Hamiltonian matrix for the periodic Haldane model

def LargeHamiltonian(N, v, w,u,t):
    ...
    for n in range(0,N):
        for m in range(0,N):
            ...
            aH[2 * N * n + 2 * m][2 * N * ((n - 1) % N) + 2 * ((m+1)%N) + 1] = w
            aH[2 * N * ((n - 1) % N) + 2 * ((m + 1) % N) + 1][2 * N * n + 2 * m] = w

            #NNN same row
            aH[2*N*n + 2*m][2*N*n + 2*((m+1)%N)] = u
            aH[2 * N * n + 2 * m +1][2 * N * n + 2 * ((m + 1) % N)+1] = u

            aH[2 * N * n + 2 * m][2 * N * n + 2 * ((m - 1) % N)] = u
            aH[2 * N * n + 2 * m + 1][2 * N * n + 2 * ((m - 1) % N) + 1] = u

            #NNN above row
            aH[2 * N * n + 2 * m][2 * N * ((n - 1) % N) + 2 * m] = u
            aH[2 * N * n + 2 * m+1][2 * N * ((n - 1) % N) + 2 * m+1] = u

            aH[2 * N * n + 2 * m][2 * N * ((n-1)%N) + 2 * ((m + 1) % N)] = u
            aH[2 * N * n + 2 * m+1][2 * N * ((n - 1) % N) + 2 * ((m + 1) % N)+1] = u
            ...
            aH[2 * N * n + 2 * m + 1][2 * N * ((n + 1) % N) + 2 * ((m - 1) % N) + 1] = u
    print(aH)
    print(min(np.absolute(np.linalg.eig(aH)[0])))

LargeHamiltonian(3,1,2,3,0)
...
aperHamiltonian(2,1,0,0,0)

def Berry(N, v, w, u, t):
    matr = Hamiltonian(N, v, w,u,t)
    pos = np.zeros((N, N, 2))
    ...
    for kx in range(1, N + 1):
        for ky in range(1, N + 1):
            ...
            for n in range(0, N):
                for m in range(0, N):
                    dat[kx - 1][ky - 1][(2 * n) * N + 2 * m] = cmath.exp(((2j * math.pi) / N) * (kx * n + ky * m)) * vec[
                        0]
                    dat[kx - 1][ky - 1][(2 * n) * N + 2 * m + 1] = cmath.exp(((2j * math.pi) / N) * (kx * n + ky * m)) * \
                                                                   vec[1]
            dat[kx - 1][ky - 1] = dat[kx - 1][ky - 1] / np.linalg.norm(dat[kx - 1][ky - 1])
    sumterm = 0
    for n in range(0, N):
        for m in range(0, N):
            sumterm = sumterm - cmath.polar(np.vdot(dat[n][m], dat[(n + 1) % N][m]) * np.vdot(dat[(n + 1) % N][m],dat[(n + 1) % N][(
                m + 1) % N]) * np.vdot(dat[(n + 1) % N][(m + 1) % N], dat[n][(m + 1) % N]) * np.vdot(dat[n][(m + 1) % N], dat[n][m]))[1]
    print(sumterm / (2 * math.pi))


Berry(N=2, v=2, w=7, u=1, t=0.0)
